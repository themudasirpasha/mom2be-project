...
export class AnganwadiComponent implements OnInit {
  ...
  processingAgents: ProcessingAgent[] = [
    { name: 'locator', label: 'Finding centers…', completed: false, active: false },
    { name: 'schemes', label: 'Checking services…', completed: false, active: false },
  ...
  ngOnInit(): void {
    this.loadAnganwadiData();
  }

  private loadAnganwadiData() {
    ...
    this.api.anganwadi().subscribe(
      ...
      (error) => {
        console.error('Error loading Anganwadi data:', error);
        this.data = [];
        this.processingAgents[3].completed = true;
        this.processingAgents[3].active = false;
        setTimeout(() => {
